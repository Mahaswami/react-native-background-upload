...
public class UploaderModule extends ReactContextBaseJavaModule {
  ...
  * Returns a promise with the string ID of the upload.
   */
    @ReactMethod
    public void startFormUpload(ReadableMap options, final Promise promise) {
      ...
      try {
        //String uploadId =
          final MultipartUploadRequest request = new MultipartUploadRequest(this.getReactApplicationContext(), url)
        ...
        if (options.hasKey("parameters")) {
          ReadableMap params = options.getMap("parameters");
          ReadableMapKeySetIterator keys = params.keySetIterator();
          while (keys.hasNextKey()) {
            String key = keys.nextKey();
            if (params.getType(key) != ReadableType.String) {
              promise.reject(new IllegalArgumentException("Params must be string key/values.  Value was invalid for '" + key + "'"));
              return;
            }
            request.addParameter(key, params.getString(key));
          }
        }
